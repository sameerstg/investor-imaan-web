...
export default function PortfolioDetailPageClient({ params }: Props) {
    const { id } = params
    ...
    const fetchPortfolio = async () => {
        ...
        const port = await getPortfolioById(id)
    ...
    const handleSaveTrade = async () => {
        ...
        try {
            ...
            if (editingTrade) {
                updatedTrade = await updateTrade(editingTrade.id, { symbol, type, quantity, price, fees })
                setTrades(trades.map(t => (t.id === updatedTrade.id ? updatedTrade : t)))
            } else {
                updatedTrade = await createTrade(id, { symbol, type, quantity, price, fees })
    ...
    return (
        <div className="p-4">
            ...
            {trades.length > 0 ? (
                    ...
                    {trades.map(t => (
                        <li key={t.id} className="border p-2 rounded flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                ...
                                <Button size="sm" variant="destructive" onClick={() => handleDeleteTrade(t.id)}>
                                    Delete
                                </Button>
                            </div>
                        </li>
                ...
                <p className="mt-4 text-gray-500">No trades found. Click "Create Trade" to add your first trade.</p>
            )}
            ...
            <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        <Button onClick={handleSaveTrade}>{editingTrade ? "Update" : "Save"}</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
